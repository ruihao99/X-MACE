...
class PermutationInvariantEncoder(torch.nn.Module):
    def __init__(self, input_dim=1, hidden_dim=16, latent_dim=128):
        super().__init__()

        self.elementwise_nn = torch.nn.Sequential(
        ...
        self.post_aggregation_nn = torch.nn.Sequential(
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.ELU(),
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.ELU(),
    ...
    def forward(self, x):
        elementwise_output = self.elementwise_nn(x)
        aggregated_output = elementwise_output.sum(dim=1)
        output = self.post_aggregation_nn(aggregated_output)
        return output
